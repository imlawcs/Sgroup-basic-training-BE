...
const createPoll = async (title, userId) => {
  ...
  if (results[0].length === 0) {
    ...
    if (createStatus) return 'Create poll successfully';else return 'Create poll fail';
  }
};
const updatePoll = async (title, userId, isLock, pollId) => {
  ...
  if (results[0].length === 0) {
    ...
    if (createStatus) return 'Update poll successfully';else return 'Update poll fail';
  }
};
const getPoll = async () => {
  try {
...
const createOption = async (title, pollId) => {
  ...
  if (results[0].length === 0) {
    ...
    if (createStatus) return 'Create option successfully';else return 'Create option fail';
  }
};
const submit = async (userId, optionId) => {
  ...
  if (result[0].length === 0 || results[0].length === 0) {
    ...
    if (createStatus) return 'Submit successfully';else return 'Submit fail';
  }
};
const unsubmit = async (userId, optionId) => {
  ...
  if (result[0].length === 0 || results[0].length === 0) {
    ...
    const createStatus = await db.query(query, [userId, optionId]);
    if (createStatus) return 'Unsubmit successfully';else return 'Unsubmit fail';
  }
};
